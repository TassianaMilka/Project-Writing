# Portfolio-22-Explanation.

# Português 

Realização descrito em português sobre alguns contextos da tecnologia entre eles:

-Computação em Nuvem sobre como solucionar contra ataque de DDos;

-Gerenciamento de Redes resoluçaõ de segmento e prevenção;

-Perícia Forense  em Segurança da Informação descrição do risco e análise;

-Criptografia sua funcionalidade;

-Segurança Defensiva Blue Team sobre Logs e Ferramentas;

-Programação para Redes sua funcionalidade.


--------------------------------------------------------------------------------------------------------------------------------

# English 

Accomplishment described in Portuguese on some contexts of technology among them:


--------------------------------------------------------------------------------------------------------------------------------

# Espanõl

Realización descrita en portugués en algunos contextos de la tecnología, entre ellos:

